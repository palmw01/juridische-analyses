# JAS-annotatieprotocol — uitvoering stap 1–15

> Uitvoeringsprotocol voor de `/jas`-skill.  
> Het visuele workflowoverzicht staat in `WORKFLOW.md`.  
> De JAS-taxonomie en annotatieregels staan in `kaders.md`.
...
**Wet `[W]` en BWB-id `[B]`**: raadpleeg `.claude/skills/shared/bwb-mapping.md` voor de volledige tabel en fallback-instructie.
...
**Begrippen-check:** lees `$CLAUDE_SKILL_DIR/begrippen-protocol.md` en voer het daarin beschreven protocol uit voor alle relevante termen in de `leden[].tekst` van artikel `[A]`. Verwerk de termen één voor één.
...
BWB-ids: zie `.claude/skills/shared/bwb-mapping.md`.
